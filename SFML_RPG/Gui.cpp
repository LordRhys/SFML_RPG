...
gui::DropDownList::DropDownList(float x, float y, float width, float height,
                                ...
                                unsigned nbrOfElements, unsigned default_index)
  : font(font), showList(false), keyTimeMax(10.f), keyTime(0.f)
{
  //unsigned nbrOfElements = sizeof(list) / sizeof(std::string);
  ...
  : keyTimeMax(10.f), keyTime(0.f)
{
  ...
  {
    this->sheet.setTextureRect(sf::IntRect(0, 0, this->sheet.getGlobalBounds().width,
...
void gui::TextureSelector::update(const float& dt, const sf::Vector2i& mousePosWindow)
{
  ...
  if (!this->hidden)
  {
    ...
    if (this->active)
    {
      this->mousePosGrid.x = (mousePosWindow.x - static_cast<int>(this->bounds.getPosition().x)) /
        static_cast<unsigned>(this->gridSize);
      this->mousePosGrid.y = (mousePosWindow.y - static_cast<int>(this->bounds.getPosition().y)) /
        static_cast<unsigned>(this->gridSize);
...
#pragma endregion TextureSelector